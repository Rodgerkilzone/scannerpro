...
useEffect(()=>{
  ...
  const getUsers=async ()=>{
    setLoading(true)
const g=query(collection(db, "users"), where("approval", "==",'approved'));
 ...
 await updateDoc(docRef, data)
  ...
  getUsers()
    ...
    <h3 style={{background:'white'}}>Search Account</h3>
     ...
     {list.length==0 && <h4 style={{background:'white'}} >NO APPROVED USERS</h4>}
     {/* {err} */}
...
<strong style={{color:'green', textTransform: 'uppercase'}}>{user.approval}</strong>
