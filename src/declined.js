 ...
 await updateDoc(docRef, data)
  ...
  getUsers()
   ...
   <h3 style={{background:'white'}}>Search Account</h3>
     ...
     {list.length==0 && <h4 style={{background:'white'}} >NO DECLINED USERS</h4>}
     {/* {err} */}
...
<div style={{margin:2,display:"flex",flexDirection:'column',width:'100%',alignItems:'center'}}>   
    ...
    /></div>
        ...
        <div style={{margin:2,display:"flex"}}>   
         ...
         <div style={{margin:2,display:"flex"}}>   
         <div >    
             <strong >Decline reason:{' '}</strong></div><div>{user.reason}</div>
         </div>
...
<strong style={{color:'red', textTransform: 'uppercase'}}>{user.approval}</strong>
